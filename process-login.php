    ...
    if(isset($_POST['email_account']) && isset($_POST['password_account'])){
        ...
        if(mysqli_num_rows($call_back_check_account) ==1){
            ...
            if($result_check_account['lock_account'] == 1 ){
                ...
                echo "<h2>ระงับบัญชีนี้เป็นเวลา $time_band_account นาที เพราะผู้ใช้กรอกรหัสผ่านผิดจำนวน $count ครั้ง</h2>";
                echo "<h3>บัญชีนี้จะถูกปลดจากการระงับเมื่อถึงเวลา $ban </h3>";
                echo '<a href="formLogin.php">กลับไปยังหน้าเข้าสู่ระบบ</a>';
                

            }elseif(password_verify($password_account,$hash)){
                $query_reset_lockin_count_account = "UPDATE account SET login_count_account = 0 WHERE email_account = '$email_account'";
                $call_back_reset = mysqli_query($connect , $query_reset_lockin_count_account);
                if($result_check_account['role_account'] == 'member'){ // rolemember
                ...
                if($result_check_account['login_count_account'] +1 >= $limit_login_account){
                    $query_lock_account = "UPDATE account SET lock_account =1 , ban_account = DATE_ADD(NOW() , INTERVAL $time_band_account MINUTE) WHERE email_account = '$email_account'";
                    $call_back_lock_account = mysqli_query($connect , $query_lock_account);
                }
                die(header('Location: formLogin.php'));//รหัสผ่านไม่ถูกต้องถ้ากรอกไม่ถูกถึงจะมาตรงนี้ 
            }

        }else{
            die(header('Location: formLogin.php'));//ไม่มี emai นี้ในระบบ
        }


    }else{
        die(header('Location: formLogin.php')); // ไม่ได้กรอกข้อมูล
    }
